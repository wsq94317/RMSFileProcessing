...
- `audit_active_cancel_unmatched.csv`: rows where `Active/Cancel` is still blank; this should normally be empty.
- `audit_active_cancel_arrival_fallback.csv`: rows where SiteMinder did not match and `Active/Cancel` was decided by Arrival List.
...
- If SiteMinder does not match, `Active/Cancel` falls back to Arrival List exact matching: present in Arrival List = `Active`, otherwise `Cancel`.
