...
- `audit_active_cancel_unmatched.csv`: master rows whose `CRS Folio #` did not match SiteMinder, so `Active/Cancel` is blank.
...
- `Active/Cancel` is checked against all SiteMinder rows by `CRS Folio # = Booking reference`; cancelled bookings become `Cancel`, booked/modified bookings become `Active`, and unmatched rows stay blank.
